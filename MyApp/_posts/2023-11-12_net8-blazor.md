...
which covers how Blazor has become a Full Stack UI Web Technology for developing any kind of Web App.
...
surprised that Blazor Apps render fast, clean HTML without needing to load the large Web Assembly runtime needed for 
...
downloaded and initialized, which would flicker as the static UI was replaced with Blazor's WASM rendered UI, whilst 
Authenticated users would experience more UI jank and delay whilst the App signs in the Authenticated User. 
...
Even better, Blazor's default **static rendering** mode results in the best UX where your App's entire Chrome UI and landing pages 
...
I expect Blazor's new **Enhanced Navigation** is likely the feature that will deliver the biggest UX improvement users will 
experience given it's enabled by default and gives traditional statically rendered Web Apps instant SPA-like navigation 
responsiveness where new pages are loaded without reloading the entire page or any of its resources. 
...
to manage client-side routing or complex state.   

It's implementation does pose some challenges in implementing some features, but we'll cover some of the approaches 
...
page whose C# DOM Event Handling require it.
...
ASP.NET Identity Pages all work without Blazor's Interactivity, they don't actually provide an implementation for providing a 
QR Code visual barcode in its 
[EnableAuthenticator.razor](https://github.com/NetCoreTemplates/blazor/blob/main/MyApp/Components/Pages/Account/Manage/EnableAuthenticator.razor)
page that mobile phones can scan to easily setup 2FA Authentication.

Whilst the placeholders are there, that implementation detail is left to us to workout how we best want to implement it within our App, 
perhaps because they don't want to force an Interactivity rendering mode in the default template.
...
is only executed once on initial page load and not re-executed when the page is navigated using Blazor's enhanced navigation.

Your options are to change every link to that page to include `data-enhance-nav="false"` to turn off Enhanced Navigation for that page,
or we need to find another way.
...
Which as you might expect works in full page reloads, but doesn't work with Blazor's Enhanced Navigation, where no JavaScript
is executed upon navigation, leaving the template as inert HTML.
...
the first time a website is accessed.
...
Which is how all Interactive features in the new [blazor-vue](https://github.com/NetCoreTemplates/blazor-vue/) template 
are implemented - ideal for building fast, SEO-friendly statically rendered Blazor Web Apps with all dynamic functionally 
progressively enhance with Vue.js:
...
Other benefits of using Vue for its Interactivity is that even its [Markdown powered Blog](https://blazor-vue.web-templates.io/blog)
can embed rich interactive Vue Components and Chart.js charts in [its Blog Posts](https://blazor-vue.web-templates.io/posts/razor-ssg-new-blog-features).
...
whose source code runs in both Blazor Server and Blazor Web Assembly Interactive modes, where the Application was 
originally developed with [Blazor Server](https://github.com/NetCoreApps/BlazorDiffusion) that we then used a 
[sync.bat](https://github.com/NetCoreApps/BlazorDiffusionWasm/blob/main/sync.bat) script to copy the source code into 
a [Blazor Web Assembly](https://github.com/NetCoreApps/BlazorDiffusionWasm) project that we could deploy and host on the Internet.

Our biggest gripe with using Blazor was the initial loading times of the website and the further delay and UI jankiness
when the App was authenticating the signed in user which we couldn't [prerender away](https://blazor-tailwind.jamstacks.net/docs/prerender).
...
We're happy with the result, much faster loading times, no UI jankiness, enhanced navigation, better SEO and a better UX overall.
...
You can compare the difference by checking out the Live Demos:
