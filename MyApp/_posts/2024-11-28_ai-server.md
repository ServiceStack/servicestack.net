...
```csharp
var api = await client.ApiAsync(new TextToImage {
...
var imageBytes = await outputImage.GetBytesFromUrlAsync(); 
```
...
### Queued Media Generation Request Example

Most other AI Server Requests are generation requests which would instead call `GetGeneration` to get
the response of a queued job, e.g:

```csharp
if (apiStatus.Response.JobState == BackgroundJobState.Completed)
{
    var api = await client.ApiAsync(new GetGeneration
    {
        RefId = uniqueRefId,
    });
}
